...
function deletePanel(removeDiv){
    $(removeDiv).parents(".panel").first().remove()
}
...
var selectNum = $("#selectNum").find("select");
var selectSelected = $("#selectNum select");
for (i = 0; i < selectNum.length; i++) {
    //alert(selectSelected.eq(i).find('option:first').attr("selected", true));;
    $("#selectResult").append("<dd class=\"selectedShow\" style=\"display:none\"><a><span></span><em></em></a>")
}
selectSelected.change(function(e){
    var selectResult = $("#selectResult > dd");
    var index = $(this).index() + 1;
    var fistTxt = $(this).children('option:first-child').text();
	//var fistTxt = $(this).children('option:first-child').val();
    var text = $(this).find("option:selected").text();
    if ($(this).get(0).selectedIndex >= 1) {
        selectResult.eq(0).hide();
        selectResult.eq(index).show();
        selectResult.eq(index).find("span").text(fistTxt + ":");
        selectResult.eq(index).find("em").text(text)
    }
    else {
        selectResult.eq(index).hide()
    }
});
$("#selectResult .selectedShow").on('click', function(){
    var index = $(this).index() - 2;
    $(this).hide();
    selectSelected.eq(index).find('option').attr("selected", false)
});
$(".accordionBtn").click(function(){
    $(".accordion dl dd").not($(this).parent().next()).fadeOut();
    $(".accordion dl dt").not($(this).parent()).removeClass("active");
